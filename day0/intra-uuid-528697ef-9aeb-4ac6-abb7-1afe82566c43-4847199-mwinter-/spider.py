...
def extraire_image_urls_local(code_source): #recupere l'url des images et le renvoi a recursif main
    
    
    pattern = r'(\S+\.(?:jpg|jpeg|png|gif|bmp).*?)'
    urls = re.findall(pattern, code_source)
...
if not re.match(r'https?://', url):
    if os.path.exists(sys.argv[nb_args -1]):
        with open(sys.argv[nb_args -1], 'r') as fichier:
            code_source = fichier.read()
        if code_source is not None:
            image_urls = extraire_image_urls_local(code_source) 
            for image in image_urls:
                image2 = image.split('"',1)[1]
                save_url_as_file(image2,path)
                print(f"Le fichier '{image2}' a été téléchargé avec succès.")
            sys.exit(1)
        sys.exit(1)
    else:
        print ("Please enter a valid URL")
